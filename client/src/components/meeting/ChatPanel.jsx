    ...
    <aside className="w-full sm:w-80 h-full bg-white border-l border-slate-200 flex flex-col z-30 shadow-2xl animate-in slide-in-from-right duration-200">
        ...
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center text-slate-400 text-sm">
            ...
            <p className="text-xs mt-1 text-slate-400">
              Send a message to start chatting with a participants!
            </p>
          </div>
        ) : (
          messages.map((msg, index) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`px-3.5 py-2.5 rounded-2xl max-w-[85%] text-sm leading-relaxed shadow-xs ${isMe ? "bg-primary text-white rounded-tr-none font-medium" : "bg-slate-100 text-slate-800 rounded-tl-none border border-slate-200 font-mendium"}`}
                >
                  {msg.text}
                </div>
              </div>
      ...
      >
        ...
        <button
          type="submit"
          disabled={!text.trim()}
          className="p-2.5 rounded-xl bg-primary hover:bg-primary-hover text-white disabled:opacity-40 transisiton-all cursor-pointer shadow-xs"
        >
